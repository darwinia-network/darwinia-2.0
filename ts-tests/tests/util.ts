...
export async function startFrontierNode(provider?: string): Promise<{
	...
	console.log("bear: --- start frontier node...");
	var web3;
	...
	const args = [
		`--chain=dev`,
		`--alice`, // Required by manual sealing to author the blocks
	...
	binary.on("error", (err) => {
		...
		console.log("bear: --- error: happened");
		process.exit(1);
	});
...
export function describeWithFrontier(title: string, cb: (context: { web3: Web3 }) => void, provider?: string) {
	describe(title, () => {
		...
		console.log("----------------before frontier node...");
